...
## I THINK THERE IS A DIFFERNT WAY TO DO THIS, LOOK BACK TO PROJECT ANCHOR CODEBASE TO SEE HOW IT WAS SOLVED
...
def load_universe(name: str) -> pd.DataFrame:
    ...
    return frame[["symbol", "group", "description"]].sort_values(["group", "symbol"]).reset_index(drop=True)
...
## NEEDS MORE DOCUMENTATION, EXPLAINING WHAT THIS IS AND WHAT ABC IS
class Provider(abc.ABC):
    ...
    def fetch(self, symbol: str, start, end=None) -> pd.DataFrame:
        ...
        if frame["adj_close"].isna().all():
            logger.warning(
                f"""[data_pulling] {self.name} gave no adj_close for {symbol}; using raw close """
                "(NOT total-return adjusted)"
            )
            frame["adj_close"] = frame["close"]

        # Trim after normalising: IBKR must request a full duration (see ibkr.py) and relies on this to cut back to the window.
        frame = frame[(frame["date"] >= start_ts) & (frame["date"] <= end_ts)].reset_index(drop=True)
        if frame.empty:
            raise CoverageError(
        ...
        # CHecking for end date coverage
